...
def get_weather_summary(city="Cheongju"):
    try:
        ...
        if res.get("cod") != 200:
            return "\U0001F324️ 현재 날씨 정보를 불러오지 못했어요. 대신 일반적인 여행 팁을 드릴게요!"

        temp = res["main"]["temp"]
        weather = res["weather"][0]["description"]

        if temp < 10:
            tip = "❄️ 꽤 쌀쌀해요. 따뜻한 외투 꼭 챙기세요!"
        elif temp < 20:
            tip = "🧥 선선한 날씨네요. 가벼운 겉옷을 챙기면 좋아요!"
        else:
            tip = "☀️ 따뜻한 날씨예요. 가볍게 떠나기 딱이에요!"

        return f"\U0001F324️ 지금 청주의 날씨는 **{weather}**, 기온은 **{temp}°C**예요.\n{tip}"
    except:
        return "\U0001F324️ 날씨 정보를 가져오는 중 문제가 발생했어요. GPT가 대신 안내할게요!"
