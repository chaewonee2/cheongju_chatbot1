    ...
    with st.spinner("청주의 아름다움을 정리 중입니다..."):
        ...
        for place in places:
            ...
            gpt_place_response = client.chat.completions.create(
                model="gpt-3.5-turbo",
                messages=[
                    {"role": "system", "content": "당신은 청주 문화유산을 소개import streamlit as st
import pandas as pd
import requests
import re
from openai import OpenAI
...
"""}
        ...
        st.session_state.messages.append({"role": "assistant", "content": final_response})하는 감성적이고 따뜻한 말투의 공손한 관광 가이드입니다. 이모지도 풍부하게 사용하세요."},
                    {"role": "user", "content": f"""
여행자에게 설렘이 느껴지도록, 따뜻하고 공손한 말투로 {place}를 소개해 주세요 ✨  
✔️ 역사적인 배경,  
✔️ 방문 시의 분위기와 계절의 어울림 🍃🌸  
